...
class BeamMonitorError(Exception):
    """Base exception class for beam monitoring errors"""

    pass


class CameraError(BeamMonitorError):
    """Exception raised for camera-related errors"""

    pass


class FittingError(BeamMonitorError):
...
class CameraSelector:
    ...
    def get_camera_info(self, index: int) -> str | None:
        ...
        try:
            ...
            try:
                ...
                name = f"Camera {index}"
                logger.warning(f"Could not get name for Camera {index}: {e!s}")

            # Take a test frame
            ret, frame = cap.read()
    
            info = f"{name} ({width}x{height})" if ret else f"{name} (No test frame)"
    ...
    def select_camera(self) -> int | None:
...
class BeamAnalyzer:
    ...
    def fit_gaussian(self, image: np.ndarray) -> dict | None:
        """
        ...
        try:
            ...
            # Perform the fit
            popt, pcov = curve_fit(
                self.gaussian_2d, xy, image.ravel(), p0=initial_guess, maxfev=1000
            )
            ...
            self.logger.error(f"Gaussian fitting failed: {e!s}")
            raise FittingError(f"Failed to fit Gaussian: {e!s}") from e
...
class BeamMonitor:
    def __init__(
        ...
        try:
            ...
            raise BeamMonitorError(f"Failed to create directories: {e!s}") from OSError
    ...
    def connect_camera(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Camera connection error: {e!s}")
            return False
    ...
    def _save_data(self, frame: np.ndarray, timestamp: datetime):
        ...
        try:
            ...
            self.logger.error(f"Error saving data: {e!s}")
    ...
    def _save_positions(self):
        ...
        try:
            ...
            self.logger.error(f"Error saving positions data: {e!s}")
    ...
    def _save_final_data(self):
        ...
        try:
            ...
            self.logger.error(f"Error saving final data: {e!s}")
        ...
        while self.running and self.points_collected < self.total_points:
            try:
                ...
                if self.points_collected % int(60 / self.interval) == 0:
                    self.logger.info(
                        f"Logging progress: {elapsed_hours:.1f} hours elapsed, "
                ...
                if num_saturated_pixels > 0:
                    self.logger.warning(
                        f"{num_saturated_pixels} saturated pixels detected! "
                        f"out of {gray.size}"
                    )
                ...
                new_row = pd.DataFrame(
                    {"timestamp": [timestamp], "x": [result["x"]], "y": [result["y"]]}
                )
                self.positions_df = pd.concat(
                    [self.positions_df, new_row], ignore_index=True
                )

                # Add to visualization queue
                self.analyzer.centers_queue.put(
                    (result["x"], result["y"], (current_time - start_time))
                )
                ...
                self.logger.error(f"Camera error: {e!s}")

                if consecutive_failures >= max_consecutive_failures:
                    self.logger.error(
                        "Too many consecutive failures, attempting camera reconnection"
                    )
                    if not self.connect_camera():
                        self.logger.error(
                            "Camera reconnection failed, waiting before retry"
                        )
                        time.sleep(self.interval)

            except FittingError as e:
                self.logger.warning(f"Fitting error: {e!s}")
                time.sleep(0.1)  # Brief pause on fitting errors

            except OSError as e:
                self.logger.error(f"Unexpected error in monitor loop: {e!s}")
                time.sleep(1)
    ...
    def update(self, frame) -> tuple[plt.Artist, ...]:
        ...
        try:
            ...
            if new_positions:
                ...
                x_pos, y_pos, elapsed_time = map(
                    np.array, zip(*new_positions, strict=False)
                )  # Convert to numpy arrays

                old_offsets = np.array(self.scatter.get_offsets())
                self.logger.debug(
                    f"Old scatter data: {old_offsets}; Type: {type(old_offsets)}"
                )

                # Ensure x_pos and y_pos are compatible with old_offsets
                new_offsets = np.vstack([old_offsets, np.column_stack([x_pos, y_pos])])
                self.logger.debug(
                    f"New scatter data: {new_offsets}; Type: {type(new_offsets)}"
                )
                ...
                self.logger.debug(
                    f"Old X series data: {old_x_time, old_x_pos}; "
                    f"Type: {type(old_x_time)}, {type(old_x_pos)}"
                )

                old_y_time, old_y_pos = map(np.array, self.line_y.get_data())
                self.logger.debug(
                    f"Old Y series data: {old_y_time, old_y_pos}; "
                    f"Type: {type(old_y_time)}, {type(old_y_pos)}"
                )
                ...
                if old_x_time.size == 0 or old_x_pos.size == 0:
                    ...
                    self.logger.debug(
                        f"New Updated X Time series: {self.line_x.get_data()}, "
                        f"Type: {type(self.line_x.get_data())}"
                    )
                else:
                    new_x_series_data = (
                        np.concatenate([old_x_time, [elapsed_time]]),
                        np.concatenate([old_x_pos, [x_pos]]),
                    )
                    self.line_x.set_data(new_x_series_data)
                    self.logger.debug(
                        f"New Updated X Time series: {new_x_series_data}, "
                        f"Type: {type(new_x_series_data)}"
                    )
                ...
                if old_y_time.size == 0 or old_y_pos.size == 0:
                    ...
                    self.logger.debug(
                        f"New Updated Y Time series: {self.line_y.get_data()}, "
                        f"Type: {type(self.line_y.get_data())}"
                    )
                else:
                    new_y_series_data = (
                        np.concatenate([old_y_time, [elapsed_time]]),
                        np.concatenate([old_y_pos, [y_pos]]),
                    )
                    self.line_y.set_data(new_y_series_data)
                    self.logger.debug(
                        f"New Updated Y Time series: {new_y_series_data}, "
                        f"Type: {type(new_y_series_data)}"
                    )
    ...
    def _start_visualization(self):
        ...
        try:
            ...
            self.logger.error(f"Error starting visualization: {e!s}")
            raise
    ...
    def _save_completion_status(self):
        ...
        try:
            with open(status_file, "w") as f:
                f.write(
                    f"Monitoring completed at: "
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                )
                f.write(
                    f"Points collected: {self.points_collected}/{self.total_points}\n"
                )
            ...
            self.logger.error(f"Error saving completion status: {e!s}")
...
def check_dependencies():
    ...
    try:
        ...
        logger.error(f"Dependency check failed: {e!s}")
        return False
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="Interval in seconds for data logging and plotting",
    )
    parser.add_argument(
        "-t", "--time", type=float, default=0.5 / 60, help="Time to log data in hours"
    )
    ...
    try:
        ...
        monitor = BeamMonitor(
            camera_index=camera_index, interval=args.interval, logging_time=args.time
        )
        monitor.start()

    except KeyboardInterrupt:
        print("\nStopping beam monitor...")
        if "monitojfijrr" in locals():
            monitor.stop()
    except RuntimeError as e:
        print(f"Error: {e!s}")
